...
#             # هنا اللعبة واقفة، بس بننقص الوقت عشان نبدأ
#             start_timer -= dt
#             if start_timer <= 0:
...
#                     player.score += 50
#                     player.speed += 100
#                     show_popup = True  # تفعيل الرسالة
#                     popup_timer = 1.5  # وقت ظهورها
        
#         if len(pellets) == 0 and game_state == GAME_PLAYING:
#             game_state = GAME_WIN
#             # RENDER
#             # مسح الشاشة (ثابت في كل الحالات)
#         glClear(GL_COLOR_BUFFER_BIT)
#         glLoadIdentity()

#         if game_state == GAME_START:
#             # هنا بنرسم كلام البداية "فقط"
#             draw_text("START GAME", 0, 0, size=70)
#             draw_text(f"Starting in {int(start_timer) + 1}...", 0, -60, size=30)

#         elif game_state == GAME_PLAYING:
#             # --- هنا بقى نحط كود الرسومات بتاعك اللي إنتي كاتباه ---
#             for w in walls:
...
#             # ونضيف "فوقيهم" رسالة الـ Popup لو موجودة
#             if show_popup:
...
#         # التحديث النهائي للشاشة (ثابت في كل الحالات)
